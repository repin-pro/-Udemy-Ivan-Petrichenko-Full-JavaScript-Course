window.addEventListener('DOMContentLoaded', function () {
    ...
    totalValue.innerHTML = 0; // Устанавливаем дефолтное значение суммы в ноль
    //обрабатываем поле ввода "На сколько дней"
    persons.addEventListener('change', function () {
        personsSum = +this.value; // записиываем то, что ввел ползователь в переменную
        total = ((daysSum + personsSum) * 4000); //считаем формулу

        //Если поле "на сколкьо дней" пустое, то выдаем ноль
        if (restDays.value == '') {
    ...
    //обрабатываем поле ввода "Количество людей"
    restDays.addEventListener('change', function () {
        daysSum = +this.value; // записиываем то, что ввел ползователь в переменную
        total = ((daysSum + personsSum) * 4000); //считаем формулу

        //Если поле "Количество людей" пустое, то выдаем ноль
        if (persons.value == '') {
            totalValue.innerHTML = 0;
    ...
    place.addEventListener('change', function () {
        if (restDays.value == '' || persons.value == '') {
            totalValue.innerHTML = 0; //Если поля не заполнены то в тотал передаем 0
        } else {
            let a = total;
            totalValue.innerHTML = a * this.options[this.selectedIndex].value; //Поулчаем значение из элемента, например value="1.5"
        }
    })
